package hostel.management.system;

import java.awt.*;
import java.awt.event.*;
import java.sql.*;
import javax.swing.*;

public class DeleteStudent extends JFrame implements ActionListener{
    
    JLabel l, l1, l2;
    JTextField t1, t2;
    JButton b1, b2;
    static String s;
    
    DeleteStudent(){
        
            super("STUDENT DATA");
            
        l = new JLabel("Delete Data");
        l.setBounds(180,-35,150,100);
        l.setFont(new Font("Tahoma", Font.ITALIC, 20));
        add(l);
        
        l1 = new JLabel("NAME");
        l1.setBounds(50,55,60,30);
        add(l1);
        
        t1 = new JTextField();
        t1.setBounds(200, 55, 200, 30);
        add(t1);
        
        l2 = new JLabel("COLLEGE ID");
        l2.setBounds(50,95,100,30); 
        add(l2);
        
        t2 = new JTextField();
        t2.setBounds(200, 95, 200, 30);
        add(t2);
        
        b1 = new JButton("Delete");
        b1.setBounds(260,130,70,30);
        b1.addActionListener(this);
        add(b1);
        
        b2 = new JButton("x");
        b2.setBounds(485,0,15,15);
        b2.addActionListener(this);
        add(b2);
                
        getContentPane().setBackground(Color.GRAY);
        setBounds(480,350,500,200);
        setLayout(null);
        setVisible(true);
    }

public void actionPerformed(ActionEvent ae){
    
    if(ae.getSource() == b1){
            String CollegeID = (String)t2.getText();
            String room = null;
            conn c = new conn();
        try{
            conn c1 = new conn();
            String str = "select room from addstudent where college_id = '"+CollegeID+"'";
            String str2 = "delete from addStudent where College_Id  = '"+CollegeID+"'";
            ResultSet rs = c1.s.executeQuery(str);
            c.s.executeUpdate(str2);            
            while(rs.next()){
            room = rs.getString("Room");
            } 
            JOptionPane.showMessageDialog(null, "Delete Sucessful");
        }catch (Exception e){
            JOptionPane.showMessageDialog(null, "WRONG CREDENTIALS");            
        }
        try{
            String str3 = "Update addroom set availability = 'Available' where room_no = '"+room+"'";            
            c.s.executeUpdate(str3);
        }catch (Exception e){}
        
        this.setVisible(false);
        new AllStudentInfo().setVisible(true);
    } else if(ae.getSource() == b2){
            this.setVisible(false);
            new AllStudentInfo().setVisible(true);
    }
}
    
public static void main(String[] args){
    new DeleteStudent();
}   
}
